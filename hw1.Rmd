...
```{r}
as.numeric(
as.factor(tibl_as_num_log_char_num$first_ten_letters) 
)
```
This doesn't work. Why?
Converting the character vecotr to a factor is shown below, along with 
a conversion for the first element in the vector.
```{r}

as.factor( tibl_as_num_log_char_num$first_ten_letters ) 
as.factor( tibl_as_num_log_char_num$first_ten_letters[1] )
class(
  as.factor( tibl_as_num_log_char_num$first_ten_letters[1] ) 
  )

```
This generates a factor with <NA>s and 0 levels... it is not possible
to convert <NA> to 
